...
Eigen::Matrix4d ICP(PointCloudT::Ptr target, PointCloudT::Ptr source, Pose init_Pose, int iter) {	
	Eigen::Matrix4d init_Trans = transform3D(init_Pose.rotation.yaw, init_Pose.rotation.pitch, init_Pose.rotation.roll,init_Pose.position.x, init_Pose.position.y, init_Pose.position.z);
	PointCloudT::Ptr transformSource (new PointCloudT);
	pcl::transformPointCloud(*source, *transformSource, init_Trans);
	...
	if (icp.hasConverged()) {
		...
		tf_matrix = tf_matrix * init_Trans;
							}
		return tf_matrix;
...
int main(){
	...
	// 1.206 accuracy when iter = 60 I am gonna cry and save this for later
	// lidar_bp.SetAttribute("upper_fov", "33");
    // lidar_bp.SetAttribute("lower_fov", "-10");
    // lidar_bp.SetAttribute("channels", "64");
    // lidar_bp.SetAttribute("range", "28");
	// lidar_bp.SetAttribute("rotation_frequency", "60");
	// lidar_bp.SetAttribute("points_per_second", "100000");
	...
	pcl::VoxelGrid<PointT> voxil_grid;
	voxil_grid.setLeafSize(0.3, 0.3, 0.3);
	...
	while (!viewer->wasStopped())
  	{ 
		...
		if(!new_scan){
			...
			voxil_grid.setInputCloud(scanCloud);
			voxil_grid.filter(*cloudFiltered);

			// TODO: Find pose transform by using ICP or NDT matching
			//pose = ....
			tf_matrix = ICP(mapCloud, cloudFiltered, pose, 58);
			pose = getPose(tf_matrix);	
			// TODO: Transform scan so it aligns with ego's actual pose and render that scan
			pcl::transformPointCloud(*cloudFiltered, *transformed_scan, tf_matrix);
